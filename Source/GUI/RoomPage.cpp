...
RoomPage::RoomPage(AeriformProcessor& p):processor(p) { controls=add<Controls>(p);startTimerHz(20); }
...
void RoomPage::resized(){controls->setBounds(0,0,getWidth()/2-6,getHeight());}
void RoomPage::paint(juce::Graphics& g) {
    ...
    g.setColour(textSecondary);g.setFont(font(12));g.drawText("Shared reflections / live stored energy",r.removeFromTop(28),juce::Justification::centredLeft);
    ...
    meter("Room energy",v.roomEnergy.load());meter("Filtered return",v.roomReturnEnergy.load());r.removeFromTop(18);
    g.setColour(textSecondary);g.setFont(font(14));g.drawFittedText("Every voice excites the same space. Output level adds the audible room; Network return sends a small, delayed amount back into the resonators. These two paths work independently.",r.removeFromTop(104),juce::Justification::centredLeft,5,1.0f);
