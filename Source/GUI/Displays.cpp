...
void NetworkDiagram::paint (juce::Graphics& g)
{
    ...
    if (value (ids::netBypass) > 0.5f || (! running[0] && ! running[1] && ! running[2]))
    {
        g.setColour (teal);
        g.drawLine (10.0f, r.getCentreY(), r.getRight() - 10.0f, r.getCentreY(), 2.0f);
        g.setFont (font (9.5f, true));
        const float textW = juce::jmax (100.0f, nodePos[2].x - nodeRadius - 20.0f - (r.getX() + 12.0f));
        g.drawText ("DIRECT EXCITER PATH", juce::Rectangle<float> (r.getX() + 12.0f, r.getBottom() - 22.0f, textW, 18.0f), juce::Justification::centredLeft);
    }
    // ---- nodes ---------------------------------------------------------------------------------------------
