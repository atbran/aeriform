...
AERIFORM_TEST(shimmer_host_audio_page_and_state) {
    auto render=[](bool on){TestHost h;h.set(ids::rack1Type,on?2:0);h.set(ids::rack1ShMix,.6f);h.noteOn(60);std::vector<float>x;CHECK(h.render(.5,&x).finite);return x;};auto a=render(false),b=render(true);double diff=0;for(size_t i=0;i<a.size();++i)diff+=std::pow(a[i]-b[i],2);CHECK(diff>1e-5);
    TestHost h;h.processor.setEditorPage(4);h.processor.setEditorSection(4,0);h.set(ids::rack1Type,2);h.set(ids::rack1ShInterval,3.5f);auto* interval=h.processor.getAPVTS().getParameter(ids::rack1ShInterval);CHECK(interval->getText(interval->getValue(),32)=="+3.50 st");h.set(ids::rack1ShInterval,7);std::unique_ptr<juce::AudioProcessorEditor> e(h.processor.createEditor());juce::Image image(juce::Image::ARGB,e->getWidth(),e->getHeight(),true);juce::Graphics g(image);e->paintEntireComponent(g,true);
    ...
    juce::MemoryBlock state;h.processor.getStateInformation(state);TestHost restored;restored.processor.setStateInformation(state.getData(),(int)state.getSize());CHECK_NEAR(restored.get(ids::rack1ShInterval),7,.01);CHECK(restored.processor.getEditorSection(4)==0);
}
