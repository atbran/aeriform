...
AERIFORM_TEST(shimmer_host_audio_page_and_state) {
    ...
    juce::MemoryBlock state;h.processor.getStateInformation(state);TestHost restored;restored.processor.setStateInformation(state.getData(),(int)state.getSize());CHECK_NEAR(restored.get(ids::shInterval),7,.01);CHECK(restored.processor.getEditorSection(4)==0);
}
